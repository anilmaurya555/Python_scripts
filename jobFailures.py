    ...
    if 'isDeleted' not in job and ('isActive' not in job or job['isActive'] is not False) and ('isPaused' not in job or job['isPaused'] is not True):
        ...
        for run in runs:
            ...
            if status == 'kFailure' or 'warnings' in run['backupRun']:
                ...
                failureCount += 1
                # report job name
                link = 'https://urldefense.com/v3/__https://*s/protection/job/*s/run/*s/*s/protection__;JSUlJQ!!OUZQVuLPJGsF!m0F3KrUGBH5EDQnMByoIPt7FsSiKTY9TS-mdmt7mhtS9mQc2C9VKrQz-QcoXGGYqmgQJ1Q$ ' % (vip, jobId, run['backupRun']['jobRunId'], run['backupRun']['stats']['startTimeUsecs'])
